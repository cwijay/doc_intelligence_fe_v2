...
function FAQPageContent({ documentId }: FAQPageContentProps) {
  const pageState = useAIContentPage(documentId, 'faq');

  return (
    <AppLayout
      noPadding
      hideSidebar
      pageTitle={pageState.document ? `FAQs: ${pageState.document.name}` : 'Document FAQs'}
    >
      <AIContentPage documentId={documentId} contentType="faq" />
    </AppLayout>
  );
}
  ...
  return (
    <AuthGuard>
      <Suspense fallback={<LoadingFallback />}>
        <FAQPageContent documentId={documentId} />
      </Suspense>
    </AuthGuard>
  );
}
